...
#------------------------Monkey报告模板分割线-------------------------------------------
def create_monkey_report(filename,test_time,test_machine,process_name,run_count,run_time,ANR_count,CRASH_count,Exception_count,cmd,info_log):
	#创建Excel文件
	workbook = xlsxwriter.Workbook(filename)
	#指定Excel表
	worksheet = workbook.add_worksheet("测试总况")
	...
	write_center(worksheet2,"A1","INFO日志", workbook)
	#write_center(worksheet2,"B1","ERRIOR日志", workbook)
