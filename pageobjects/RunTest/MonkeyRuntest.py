...
sys.path.append("..")
from monkey.readconfig import run_shell
from monkey.analysis import AnalysisLog
from monkey.Monkey_Base import AndroidBaseOperation
from lib.Excel_reportx import create_monkey_report

def start():
	phoneMsg = AndroidBaseOperation()
	print("开始")
	cmd,logpath,info_log = run_shell() #获得执行语句
	...
	while True:
		with open(logpath, encoding='utf-8') as monkeylog:
			...
			if monkeylog.read().count('Monkey finished') > 0:
				print("测试完成")
				break
	And_version,phone_model,phone_name = phoneMsg.getModel() #安装系统版本 手机品牌 手机名
